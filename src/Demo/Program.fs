...
module Octbuild =
    ...
    let test() =
        ...
        el.addEventListener_change(fun e ->
            let f = el.files.[0]
            let import (dbName : string) (fmt : Pts.Token[]) (f : File) =
                promise {
                    let name = f.name
                    Log.start "[Octree] import %s" name
                    do! LocalDatabase.destroy dbName
                    let startTime = performance.now()
            ...
            let fmt = [| Pts.Token.PositionX; Pts.Token.PositionY; Pts.Token.PositionZ; Pts.Token.Skip; Pts.Token.BlueByte; Pts.Token.GreenByte; Pts.Token.RedByte |]
