...
type GLSLVersion (major : int, minor : int, build : int, suffix : string) =
    member x.Major = major
    member x.Minor = minor
    member x.Build = build
    member x.Suffix = suffix

    new(major : int, minor : int) = GLSLVersion(major, minor, 0, "")

    override x.GetHashCode() =
        HashCode.Combine(major, minor, build, Unchecked.hash suffix)

    override x.Equals o =
        match o with
        | :? GLSLVersion as o -> o.Major = major && o.Minor = minor && o.Build = build && o.Suffix = suffix
        | _ -> false
...
type Backend private(config : Config) =
    ...
    static let table = Dict<Config, Backend>(Unchecked.hash, Unchecked.equals)

    member x.Config = config

    static member Create (config : Config) =
        table.GetOrCreate(config, fun config ->
            Backend(config)
        )
    ...
    override x.TryGetIntrinsicMethod (c : MethodInfo) =
        match c with
            ...
            | TextureLookup fmt when config.version.Major > 1 -> Some (CIntrinsic.tagged fmt)
            | TextureLookupOld fmt when config.version.Major <= 1 -> Some (CIntrinsic.tagged fmt)
...
module Assembler =
    ...
    let reservedNames =
        ...
        Log.warn "asdasdasdasdasdasdsad"
        let rx = Regex("^(" + str + ")$")
        Log.warn "asfdasdasd"
        rx
            ...
            match e with
                ...
                | CCallIntrinsic(ret, func, args) ->
                    match func.additional with
                        | null -> ()
                        | exts -> do! State.modify (fun s -> { s with requiredExtensions = Set.union s.requiredExtensions (unbox<Set<string>> exts) })
    ...
    let private uniformLayout (config : Config) (isUniformBuffer : bool) (decorations : list<UniformDecoration>) (set : int) (binding : int) =
        ...
        let decorations =
            if isUniformBuffer && config.stdLayout then "std140" :: decorations
            else decorations
                    ...
                    state {
                        ...
                        match name with
                                ...
                                let! buffers =
                                    fields |> List.mapS (fun (d,name,_,ct) ->
                                        state {
                                            ...
                                            let prefix = uniformLayout config false [] set binding
                            ...
                            | Some bufferName when config.createUniformBuffers ->
                                ...
                                let prefix = uniformLayout config true [] set binding
                                                ...
                                                | Some (t, cnt) ->
                                                    match t with
                                                        | GLSLTextureType.GLSLSampler samplerType -> 
                                                            ...
                                                            prefix <- uniformLayout config false decorations set binding
...
//                                                | _ -> ""
                    ...
                    let isFirstStage = Option.isNone prevStage
                    let isLastStage = Option.isNone nextStage
                    ...
                    let layoutParams = 
                        match kind with
                            | ParameterKind.Input | ParameterKind.Output when config.createInputLocations ->
                                if isFirstStage && kind = ParameterKind.Input then
                                    [ sprintf "location = %d" location]
                                elif isLastStage && kind = ParameterKind.Output then
                                    [ sprintf "location = %d" location]
                                elif config.createInternalLocations then    
                                    [ sprintf "location = %d" location]
                                else
                                    []
                            | _ ->
...
//
                    ...
                    let prefix, suffix =
                        ...
                        | InOutMode.AttributeVarying -> 
                            match kind with
                                | ParameterKind.Input -> 
                                    if isFirstStage then "attribute ", ""
                                    else "varying ", ""
        ...
        state {
            ...
            let prefix = 
                match stages.self with
                    ...
                    | ShaderStage.Compute ->
                        let localSize = e.cDecorations |> List.tryPick (function EntryDecoration.LocalSize s when s.AllGreater V3i.Zero -> Some s | _ -> None) 
                        [
    ...
    let assemble (backend : Backend) (m : CModule) =
        ...
        let definitions =
            state {
                ...
                let versionstr =
                    if String.IsNullOrWhiteSpace c.version.Suffix then sprintf "%d%d0" c.version.Major c.version.Minor
                    else sprintf "%d%d0 %s" c.version.Major c.version.Minor c.version.Suffix
        ...
        let state = ref <| AssemblerState.ofConfig c
