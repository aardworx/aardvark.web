...
module CType =
    let private primitiveTypes =
        HMap.ofList [
            ...
            typeof<nativeint>, CType.CInt(true, 4)
            ...
            typeof<unativeint>, CType.CInt(false, 4)
...
module CLExpr =
    ...
    let rec ofExprSafe (e : CExpr) =
        match e with
            ...
            | CField(t, e, name) -> 
                ofExprSafe e |> Option.map (fun e ->
                    CLField(t, e, name)
                )

            | CItem(t, e, index) -> 
                CLItem(t, e, index) |> Some
            ...
            | CMatrixElement(t, e, r, c) ->
                ofExprSafe e |> Option.map (fun e ->
                    CLMatrixElement(t, e, r, c)
                )
                    
            | CReadInput(k,t,n,i) -> 
                CLInput(k,t,n,i) |> Some
...
module CUniform =
    let mergeMany (groups : seq<list<CUniform>>) =
        let all = Dict(Unchecked.hash, Unchecked.equals)
