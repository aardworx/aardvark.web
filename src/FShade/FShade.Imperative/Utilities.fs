...
module MissingInBase =
    ...
    module Dict =
        let ofSeq (l : seq<'k * 'v>) =
            let r = Dict(Unchecked.hash, Unchecked.equals)
            for (k, v) in l do
                r.[k] <- v
            r
        let ofList (l : list<'k * 'v>) =
            ofSeq l
        let ofArray (l : array<'k * 'v>) =
            ofSeq l
    ...
    let (|VectorOf|_|) (t : Type) =
        if t = typeof<V2d> then Some (2, typeof<float>)
        elif t = typeof<V3d> then Some (3, typeof<float>)
        elif t = typeof<V4d> then Some (4, typeof<float>)
        elif t = typeof<V2i> then Some (2, typeof<int>)
        elif t = typeof<V3i> then Some (3, typeof<int>)
        elif t = typeof<V4i> then Some (4, typeof<int>)
        else None
        
    let (|MatrixOf|_|) (t : Type) =
        if t = typeof<M22d> then Some (V2i(2,2), typeof<float>)
        elif t = typeof<M23d> then Some (V2i(3,2), typeof<float>)
        elif t = typeof<M24d> then Some (V2i(4,2), typeof<float>)
        elif t = typeof<M32d> then Some (V2i(2,3), typeof<float>)
        elif t = typeof<M33d> then Some (V2i(3,3), typeof<float>)
        elif t = typeof<M34d> then Some (V2i(4,3), typeof<float>)
        elif t = typeof<M42d> then Some (V2i(2,4), typeof<float>)
        elif t = typeof<M43d> then Some (V2i(3,4), typeof<float>)
        elif t = typeof<M44d> then Some (V2i(4,4), typeof<float>)
        else None
...
module ReflectionPatterns =
    ...
    let (|MatrixValue|_|) (v : obj) =
        match v with
            | :? M22d as m -> Some (typeof<float>, [| m.M00; m.M01; m.M10; m.M11 |])
            //| :? IMatrix as m ->
            //    let values =
            //        [|
...
//                
    ...
    [<AbstractClass>]
    type State<'s, 'a>() =
        abstract member Run : ref<'s> -> 'a

    //[<AbstractClass>]
    //type UnitState<'s>() =
    //    inherit State<'s, unit>()
    //    override x.Run(s) = x.RunUnit(s)
...
module StateExtensions =
    ...
    module List =
        let mapS (f : 'a -> State<'s, 'b>) (m : list<'a>) : State<'s, list<'b>> =
        ...
        let chooseS (f : 'a -> State<'s, Option<'b>>) (m : list<'a>) : State<'s, list<'b>> =
            { new State<'s, list<'b>>() with
                override x.Run(s : ref<'s>) =
                    let res = System.Collections.Generic.List<'b>()
                    for v in m do
                        match f(v).Run(s) with
                        | Some r -> res.Add(r)
                        | None -> ()
                    Seq.toList res
            }
...
module Helpers = 
    ...
    let rec typeName (t : Type) =
        typeNameCache.GetOrCreate(t, fun t ->
            if FSharpType.IsTuple(t) then
                ...
                if not (isNull t.DeclaringType) then 
    ...
    let methodName (mi : MethodBase) =
        methodNameCache.GetOrCreate(mi, fun mi -> 
            match mi with
                | :? MethodInfo as mi -> 
                    let selfName =
