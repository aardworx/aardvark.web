...
module Compiler =
    ...
    type Backend() =
        let intrinsicFunctions = Dict<MethodBase, Option<CIntrinsic>>(Unchecked.hash, Unchecked.equals)
        let intrinsicTypes = Dict<Type, Option<CIntrinsicType>>(Unchecked.hash, Unchecked.equals)
        ...
        member x.TryGetIntrinsic (m : MethodBase) =
            intrinsicFunctions.GetOrCreate(m, fun m ->
                match m with
        ...
        member x.TryGetIntrinsic (t : Type) =
            intrinsicTypes.GetOrCreate(t, fun t ->
    ...
    module Constructors =
        ...
        let cache = Dict<IBackend * Type, FunctionDefinition>(Unchecked.hash, Unchecked.equals)
        let unionCache = Dict<IBackend * Type, hmap<UnionCaseInfo, FunctionDefinition>>(Unchecked.hash, Unchecked.equals)
        let ctorCache = Dict<IBackend * ConstructorInfo, FunctionDefinition>(Unchecked.hash, Unchecked.equals)

        let tuple (b : IBackend) (t : Type) =
            cache.GetOrCreate((b,t), fun (b,t) ->
                let cName = 
                    "new_" + typeName t
        ...
        let union (b : IBackend) (t : Type) =
            unionCache.GetOrCreate((b,t), fun (b,t) ->
        ...
        let custom (b : IBackend) (ctor : ConstructorInfo) =
            ...
            let preprocessCtor (e : Expr) =
                let rec preprocess (this : Var) (arg : Var) (e : Expr) =
                    match e with
                        ...
                        | Sequential(NewObject(ctor, args), rest) ->
            ...
            ctorCache.GetOrCreate((b,ctor), fun (b, ctor) ->
    ...
    module Helpers = 
        ...
        let rec tryGetBuiltInMethod (b : IBackend) (mi : MethodInfo) (args : list<CExpr>) =
            ...
            match mi, args with
                ...
                | Method("get_Item", [VectorOf _; _]), [v;i] -> CVecItem(ct, v, i) |> Some


                | Method("get_X", [VectorOf _]), [v]  -> CVecSwizzle(ct, v, [CVecComponent.X]) |> Some
                | Method("get_Y", [VectorOf _]), [v]  -> CVecSwizzle(ct, v, [CVecComponent.Y]) |> Some
                | Method("get_Z", [VectorOf _]), [v]  -> CVecSwizzle(ct, v, [CVecComponent.Z]) |> Some
                | Method("get_W", [VectorOf _]), [v]  -> CVecSwizzle(ct, v, [CVecComponent.W]) |> Some
        ...
        state {
            ...
            let getVar (kind : ParameterKind) (name : string) (typ : Type) (idx : Option<Expr>) =
                ...
                match HMap.tryFind key variables with
                    ...
                    | None ->
                        let suffix =
                            match idx with
                                | Some idx -> string idx //Expr.ComputeHash idx
                                | None -> ""
            ...
            if not usesUniform && HSet.isEmpty free then
                ...
                let parameters, newState = 
                    ...
                    parameters.Run(oldState), oldState.Value.moduleState 
